...
namespace TownOfUs.Roles
{
    public abstract class BaseLover : BaseRole
    {
        ...
        {
            return base.Criteria() || PlayerControl.LocalPlayer.PlayerId == OtherLover.Player.PlayerId;
        }
        ...
        internal override bool HasNotReachedEndCondition(ShipStatus __instance)
        {
            ...
            if (OtherLover.Player.Data.IsDead) return false;

            var aliveCount = PlayerControl.AllPlayerControls.ToArray().Count(player => !player.Data.IsDead);

            if (HasLovingImpostor && aliveCount == 4)
            {
                // Prevent game from ending if the alive players consist of
                // lover, loving impostor, impostor, and something else.
                return false;
            }
        ...
        private bool FourPeopleLeft()
        {
            ...
            var lover2 = OtherLover.Player;
            {
                return !lover1.Data.IsDead && !lover2.Data.IsDead &&
                       alives.Count() == 4 && HasLovingImpostor;
            }
        }
        ...
        public void Win()
        {
            var roles = RoleService.Instance.GetRoles();
            if (roles.GetRoles<Jester>().Any(jester => jester.VotedOut)) return;
            /*var lover1 = Player;
            var lover2 = OtherLover.Player;
